...
class JsonSchema {
	...
	constructor(model, options = {}) {
		...
		this.options.regexAttributeBlackList = this.options.regexAttributeBlackList || [];

		this.options.appendToType = this.options.appendToType || {};

		this.customSchema = this.options.customSchema || {};

		this.options.additionalProps = this.options.additionalProps || {};
	}
	...
	selectAttributes() {
		let attributes = this.options.selectAttributes ?
			...
			Object.keys(this.model.rawAttributes).filter(attribute =>
				 !this.options.regexAttributeBlackList.some(regex => new RegExp(regex).test(attribute)));
	...
	generatePropertySchema({ type, key }) {
		const appendToTypeKeys = Object.keys(this.options.appendToType);
		let typeValue = typeMap[type];

		if (appendToTypeKeys.length) {
			const match = appendToTypeKeys.some(lookup => {
				return new RegExp(lookup).test(key);
			});

			if (match && this.options.appendToType[key]) {
				typeValue = [typeValue, ...this.options.appendToType[key]];
			}
		}
